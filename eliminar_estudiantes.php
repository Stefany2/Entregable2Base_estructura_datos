...
<div class="container">
  ...
  <h2 class="text-center">Eliminar Estudiante</h2>
  <form id="formulario-eliminar-estudiante" class="mb-4">
    <div class="form-group">
      <label for="idEliminar">ID del Estudiante:</label>
...
<script>
  ...
  document.getElementById('formulario-eliminar-estudiante').addEventListener('submit', function(event) {
    event.preventDefault();
    
    const id = document.getElementById('idEliminar').value;

    // Enviar la solicitud DELETE a la API para eliminar al estudiante
    fetch(`http://127.0.0.1:8000/api/estudiantes/${id}`, {
      method: 'DELETE',
    })
    .then(response => response.json())
    .then(data => {
      if (data.status === 200) {
        ...
        // Limpiar el formulario después de eliminar al estudiante
        document.getElementById('formulario-eliminar-estudiante').reset();
      } else {
        console.error('Error al eliminar el estudiante:', data.message);
